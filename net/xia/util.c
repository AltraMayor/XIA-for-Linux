...
int xia_test_addr(const struct xia_addr *addr)
{
	...
	/* An empty DAG is not a valid address. */
	if (n < 1)
		return XIAEADDR_EMPTY;
...
int xia_ntop(const struct xia_addr *src, char *dst, int dstlen)
{
	int i;
	int tot = 0;
	char *p = dst;
	...
	for (i = 0; i < XIA_NODES_MAX && left > 0; i++) {
		...
		const u8 *edge = row->s_edge.a;
		char e0 = ch_edge[edge[0] & 0x1f];
		char e1 = ch_edge[edge[1] & 0x1f];
		char e2 = ch_edge[edge[2] & 0x1f];
		char e3 = ch_edge[edge[3] & 0x1f];
		char *se0 = edge[0] & XIA_CHOSEN_EDGE ? ">" : "";
		char *se1 = edge[1] & XIA_CHOSEN_EDGE ? ">" : "";
		char *se2 = edge[2] & XIA_CHOSEN_EDGE ? ">" : "";
		char *se3 = edge[3] & XIA_CHOSEN_EDGE ? ">" : "";
		char *sep = i > 0 ? ":" : "";
		int count;
		if (ty == XIDTYPE_NAT)
			break;
...
static inline void next(const char **pp, int *pleft)
{
	(*pp)++;
	(*pleft)--;
}

static int read_sep(const char **pp, int *pleft, char sep)
{
	if (*pleft <= 0 || **pp != sep)
		return -1;
	next(pp, pleft);
...
static int read_be32(const char **pp, int *pleft, __be32 *value)
{
	u32 result = 0;
	int i = 0;
...
static int read_type(const char **pp, int *pleft, xid_type_t *pty)
{
	BUILD_BUG_ON(sizeof(xid_type_t) != 4);

	if (read_be32(pp, pleft, pty))
		return -1;
	return 0;
}

static int read_xid(const char **pp, int *pleft, u8 *xid)
{
	int i;
	...
	for (i = 0; i < 5; i++) {
		if (read_be32(pp, pleft, pxid++))
			return -1;
	}
	return 0;
}

static int read_edges(const char **pp, int *pleft, u8 *edge, int ignore_ce)
{
	int i;
...
static int read_row(const char **pp, int *pleft, struct xia_row *row,
	int ignore_ce)
{
...
int xia_pton(const char *src, int srclen, struct xia_addr *dst, int ignore_ce)
{
	const char *p = src;
	int left = srclen;
	int i = 0;
